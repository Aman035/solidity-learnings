// SPDX-License-Identifier: MIT
pragma solidity ^0.8.19;

import {Test, console} from "forge-std/Test.sol";
import {MoodNFT} from "../../src/MoodNFT.sol";
import {DeployMoodNFT} from "../../script/DeployMoodNFT.s.sol";

contract MoodNFTTest is Test {
    MoodNFT public moodNFT;
    address public USER = makeAddr("user");
    string public constant HAPPY_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";

    function setUp() public {
        DeployMoodNFT deployer = new DeployMoodNFT();
        moodNFT = deployer.run();
    }

    ////////////////
    // Initial State
    ////////////////
    function testNFTName() public {
        assertEq(moodNFT.name(), "Mood NFT");
    }

    function testNFSymbol() public {
        assertEq(moodNFT.symbol(), "MN");
    }

    function testGetTokenCounter() public {
        assertEq(moodNFT.getTokenCounter(), 0);
    }

    function testGetHappySvgImageUri() public {
        assertEq(moodNFT.getHappySvgImageUri(), HAPPY_SVG_IMAGE_URI);
    }

    function testGetSadSvgImageUri() public {
        assertEq(moodNFT.getSadSvgImageUri(), SAD_SVG_IMAGE_URI);
    }

    ////////////////
    // Mint
    ////////////////
    function testNFTMint() public {
        vm.prank(USER);
        moodNFT.mint();
        assertEq(moodNFT.balanceOf(USER), 1);
        console.log(moodNFT.tokenURI(0));
    }

    ////////////////
    // FlipMood
    ////////////////
    function testFlipMoodRevertsWhenNotOwnerOrApproved() public {
        vm.prank(USER);
        moodNFT.mint();
        vm.prank(makeAddr("not owner"));
        vm.expectRevert(MoodNFT.MoodNFT__NonOwnerCannotFlipMood.selector);
        moodNFT.flipMood(0);
    }

    function testFlipMoodForOwner() public {
        vm.startPrank(USER);
        moodNFT.mint();
        moodNFT.flipMood(0);
        vm.stopPrank();
        assert(moodNFT.getTokenMood(0) == MoodNFT.Mood.SAD);
    }

    function testFlipMoodForApproved() public {
        // Setup
        address nonOwner = makeAddr("not owner");

        vm.startPrank(USER);
        moodNFT.mint();
        moodNFT.approve(nonOwner, 0);
        vm.stopPrank();

        // Action
        vm.prank(nonOwner);
        moodNFT.flipMood(0);

        // Assert
        assert(moodNFT.getTokenMood(0) == MoodNFT.Mood.SAD);
    }
}
